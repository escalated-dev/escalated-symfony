...
class NewsletterRenderer
{
    ...
    private function resolvePath(string $path, $contact, NewsletterDelivery $delivery): string
    {
        ...
        if ('contact.name' === $path) {
            return $name;
        }
        if ('contact.first_name' === $path) {
            return explode(' ', $name)[0] ?? '';
        }
        if ('contact.email' === $path) {
            return $email;
        }
        if ('unsubscribe_url' === $path) {
            return $this->unsubscribeUrl($delivery);
        }
        if ('view_in_browser_url' === $path) {
            return $this->viewInBrowserUrl($delivery);
        }
    ...
    private function rewriteLinks(string $html, NewsletterDelivery $d): string
    {
        ...
        return preg_replace_callback(
            ...
            function ($m) use ($d, $unsub, $view) {
                ...
                if (str_starts_with($href, $unsub) || str_starts_with($href, $view)) {
                    return $m[0];
                }
